...
async def get_discussions(
    ...
    if discussions:
        ...
        has_replies_resp = (
            db.table("discussions").select("parent_id").in_("parent_id", all_ids).execute()
        )
        ids_with_replies = {
            row["parent_id"] for row in (has_replies_resp.data or []) if row.get("parent_id")
        }
        discussions = [
            d for d in discussions if d.get("content") != "[deleted]" or d["id"] in ids_with_replies
        ]
...
async def get_discussion(
    ...
    if discussion.get("content") == "[deleted]":
        replies_resp = (
            db.table("discussions").select("id").eq("parent_id", str(discussion_id)).execute()
        )
...
async def delete_discussion(
    ...
    if existing.data[0]["user_id"] != current_user["id"] and current_user.get("role") not in (
        "admin",
        "moderator",
    ):
...
async def block_discussion(
    ...
    while to_visit:
        ...
        child_ids = [
            ...
            if row.get("id") and row["id"] not in visited
        ]
        if not child_ids:
            break
