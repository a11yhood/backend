-- Recompute GitHub source ratings using the same log-based mapping as the scraper.
-- Adapter formula: round(clamp(log10(stars), 1.0, 5.0), 2), with stars <= 0 => NULL.
...
WITH recalculated AS (
  SELECT
    ...
    CASE
      ...
      ELSE ROUND(
        LEAST(
          GREATEST(LOG(10, p.source_rating_count::NUMERIC), 1.0),
          5.0
        ),
        2
      )::NUMERIC(3,2)
